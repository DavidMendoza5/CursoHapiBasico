...
async function getUser({ query }, h){
    ...
    const totalPages = Math.ceil(totalUsers / limit) //Ceil redondea siempre hacia arriba. lo necesitamos porque si sobre un objeto, se mostrará solito en una página
...
async function createUser({ payload }, h){
    let { active = true } = payload //Es una forma de acceder a ambos, la H funciona como un valor por default
    ...
    .code(201)
    .header('Content-Type', 'application/json')
    //return h.response('Hola '+ req.payload.name)
    //return h.response(`${name} ${genero} ${email}`) //Creado con interpolación
}
